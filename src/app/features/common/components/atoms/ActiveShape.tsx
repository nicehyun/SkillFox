...
const ActiveShape = ({ props, color = "primary" }: IActiveShapeProps) => {
  const activeColor = color === "primary" ? "#092C4C" : "#F2994A";
  ...
  return (
    <g>
      <text x={cx} y={cy} dy={8} textAnchor="middle" fill={activeColor}>
        {payload.name}
      </text>
      <Sector
        cx={cx}
      ...
      <path
        ...
        fill="none"
      />
      <circle cx={ex} cy={ey} r={2} fill={activeColor} stroke="none" />
      <text
        x={ex + (cos >= 0 ? 1 : -1) * 12}
        y={ey}
